...
public class XRInteractorLineModified : MonoBehaviour, IXRCustomReticleProvider
{
	...
	void UpdateLineVisual()
	{
		...
		else
		{
			...
			{
				System.Array.Copy(m_TargetPoints, m_RenderPoints, m_NoTargetPoints);
		...
		if (isValidTarget)
		{
			if (playerController.CheckValidTeleportPoint(m_ReticlePos))
			{
		...
		System.Array.Copy(m_RenderPoints, m_PreviousRenderPoints, m_NoRenderPoints);
